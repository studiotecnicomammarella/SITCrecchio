...
// ol-ext geolocate gps
	...
	// Set info
	geolocelement.innerHTML = "Attiva GPS dispositivo"
...
// Popup WMS-WFS Query
	var querywmswfs = new ol.control.Toggle({
		...
		title: 'Interroga layer WMS-WFS',
		className: 'querywmswfs',
		onToggle: function(active) {
		  	if (active) {
				if (!Array.isArray(wms_layers) || wms_layers.length === 0) {
					alert("Nessun layer WMS interrogabile presente.");
				...
				if (visibleWmsLayers.length === 0) {
					alert("Layer WMS interrogabili rilevati, ma nessuno è attualmente visibile.");
					querywmswfs.setActive(false);
					return;
				}
...
// measurement
	...
	map.on('pointermove', function (evt) {
	  ...
	  if (measuring) {
		/** @type {string} */
		//modify measure:text
		var helpMsg = "Clicca per iniziare la misurazione";
		if (sketch) {
	...
	var measurementOption = [
	  { value: "LineString", description: "Lunghezza" },
	  { value: "Polygon", description: "Area" },
	  { value: "Circle", description: "Raggio" }
	];
	// Aggiungere le opzioni alla select
	 ...
	 * @type {string}
	 */
	var continueLineMsg = "1 clic continua, 2 clic chiudi";
	 ...
	 * @type {string}
	 */
	var continuePolygonMsg = "1 clic continua, 2 clic chiudi";
	 ...
	 * @type {string}
	 */
	var continueCircleMsg = "1 clic per chiudere";
...
/*		
	...
	editBar.getInteraction('DrawPoint').on('change:active', function(e){
	  tooltip.setInfo(e.oldValue ? '' : 'Clicca sulla mappa per posizionare un punto...');
	});
	
	editBar.getInteraction('DrawLine').on(['change:active','drawend'], function(e){
	  tooltip.setFeature();
	  tooltip.setInfo(e.oldValue ? '' : 'Clicca sulla mappa per iniziare a disegnare una linea...');
	});
	editBar.getInteraction('DrawLine').on('drawstart', function(e){
	  tooltip.setFeature(e.feature);
	  tooltip.setInfo('Clicca per continuare a disegnare la linea...');
	});
	editBar.getInteraction('DrawPolygon').on('drawstart', function(e){
	  tooltip.setFeature(e.feature);
	  tooltip.setInfo('Clicca per continuare a disegnare...');
	});
	editBar.getInteraction('DrawPolygon').on(['change:active','drawend'], function(e){
	  tooltip.setFeature();
	  tooltip.setInfo(e.oldValue ? '' : 'Clicca sulla mappa per iniziare a disegnare...');
	});
...
// popup Coordinates position control
	...
	document.addEventListener('DOMContentLoaded', function() {
		setTimeout(function() {
			// Zoom, Rotate, FullScreen nativi OpenLayers
			var zoomIn = document.querySelector('.ol-zoom-in');
			if (zoomIn) zoomIn.title = 'Ingrandisci';
			var zoomOut = document.querySelector('.ol-zoom-out');
			if (zoomOut) zoomOut.title = 'Riduci';
			var rotateReset = document.querySelector('.ol-rotate button');
			if (rotateReset) rotateReset.title = 'Azzera rotazione';
			var fsBtn = document.querySelector('.ol-full-screen button');
			if (fsBtn) fsBtn.title = 'Schermo intero';

			// Strumenti editBar — SOLO tooltip title
			function translateEditBar() {
				var editBarMap = {
					'ol-drawpoint':    'Disegna punto',
					'ol-drawline':     'Disegna linea',
					'ol-drawpolygon':  'Disegna poligono',
					'ol-drawregular':  'Disegna forma regolare',
					'ol-drawhole':     'Disegna foro',
					'ol-select':       'Seleziona',
					'ol-info':         'Info',
					'ol-delete':       'Elimina',
					'ol-offset':       'Offset',
					'ol-transform':    'Trasforma (sposta/ruota/scala)',
					'ol-split':        'Dividi'
				};
				Object.keys(editBarMap).forEach(function(cls) {
					document.querySelectorAll('.ol-editbar .' + cls + ' button').forEach(function(btn) {
						btn.title = editBarMap[cls];
					});
					document.querySelectorAll('.ol-editbar .' + cls).forEach(function(div) {
						div.title = editBarMap[cls];
					});
				});
			}
			translateEditBar();
			var editBarEl = document.querySelector('.ol-editbar');
			if (editBarEl) {
				var ebObs = new MutationObserver(translateEditBar);
				ebObs.observe(editBarEl, { childList: true, subtree: true });
			}
		}, 500);
	});

	// Posiziona "Mostra coordinate" come fratello di .querywmswfs (pulsante WMS)
	setTimeout(function() {
		var _pcCtrl = document.querySelector('.popup-coordinates');
		var _wmsCtrl = document.querySelector('.querywmswfs');
		if (_pcCtrl && _wmsCtrl && _wmsCtrl.parentNode) {
			_wmsCtrl.parentNode.insertBefore(_pcCtrl, _wmsCtrl.nextSibling);
			function positionPc() {
				var wmsRect = _wmsCtrl.getBoundingClientRect();
				var parentRect = _wmsCtrl.parentNode.getBoundingClientRect();
				_pcCtrl.style.position = 'absolute';
				_pcCtrl.style.top = (wmsRect.top - parentRect.top) + 'px';
				_pcCtrl.style.left = (wmsRect.right - parentRect.left + 4) + 'px';
				_pcCtrl.style.marginBottom = '0';
			}
			positionPc();
...
// mouse position coordinates
	...
	mousePositionButton.on('click', function() {
	  var newEpsg = prompt('Visualizza le coordinate nel SR desiderato inserendo il codice EPSG (es. 4326):');
	  if (isValidEpsg(newEpsg)) {
		...
		alert('Codice EPSG non valido. Inserisci un codice EPSG valido.');
	  }
	});
...
// ol-ext print dialog control
	...
	// Set button title
	printbuttontitle.attr('title', 'Stampa');
...
// ol-ext layerswitcher
	...
	// Set button title
	layerswitcherbuttontitle.attr('title', 'Layer');
	...
	layerSwitcher.on('drawlist', function (e) {
		...
		// Traduzione titoli layerup (su/giù) e layerExtent (zoom estensione)
		// Posizionate PRIMA del return nel symbology switcher così funzionano su TUTTI i layer
		var _layerupDiv = li.querySelector('.layerup');
		if (_layerupDiv) _layerupDiv.title = 'Sposta su / giù';
		var _layerExtDiv = li.querySelector('.layerExtent');
		if (_layerExtDiv) _layerExtDiv.title = 'Zoom estensione layer';
...
// StreetView
	var streetViewOptions = {
		...
		i18n: {dragToInit: 'StreetView - Trascinami sulla mappa'}
	}
